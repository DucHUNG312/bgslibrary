...
__global__ void cuFrameDifference_kernel_8u_c3r(
  ...
  const int ch[3] = {y * in_step + 3 * x, y * in_step + 3 * x + 1,
                     y * in_step + 3 * x + 2};
  float gray = 0.114f * abs(img_input[ch[0]] - bg_input[ch[0]]) +
               0.587f * abs(img_input[ch[1]] - bg_input[ch[1]]) +
               0.299f * abs(img_input[ch[2]] - bg_input[ch[2]]);
...
void cuFrameDifference_8u_c3r(const cv::cuda::GpuMat &img_input,
  ...
  cuFrameDifference_kernel_8u_c3r<<<blocks, NUM_THREADS>>>(
      ...
      static_cast<int>(img_input.step), static_cast<int>(img_output.step),
      threshold);
