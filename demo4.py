##############################################
# Demo4: FrameDifference CPU vs cuFrameDifference GPU comparison
# Usage:
#   python demo4.py [input_video]
# Shows side-by-side: [Input | FrameDifference CPU mask | cuFrameDifference GPU mask]
# Press 'q' to quit
##############################################
...
print("Layout: [Input | FrameDifference CPU mask | cuFrameDifference GPU mask]")
print("Press 'q' to quit")

cpu_bgs = bgs.FrameDifference()
gpu_bgs = bgs.cuda.FrameDifference()
...
while True:
    ...
    cv2.putText(
        gpu_disp,
        f"cuFrameDiff GPU {gpu_ms:.1f} ms",
        (10, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (0, 220, 255),
    ...
    cv2.imshow(
        "FrameDifference CPU vs cuFrameDifference GPU",
        np.hstack([frame, cpu_disp, gpu_disp]),
    )

    if frame_count % 30 == 0 and gpu_total_ms > 0:
        print(
...
if frame_count > 0:
    ...
    print(f"CPU FrameDifference  avg: {cpu_total_ms/frame_count:.2f} ms/frame")
    print(f"CUDA FrameDifference avg: {gpu_total_ms/frame_count:.2f} ms/frame")
    if gpu_total_ms > 0:
